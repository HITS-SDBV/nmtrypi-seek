...
Commontator.configure do |config|
  ...
  config.current_user_proc = lambda { |controller| User.current_user }
  ...
  config.user_name_proc = lambda { |user| user.nil? ? I18n.t('commontator.anonymous') : user.display_name }
  ...
  config.user_link_proc = lambda { |user, app_routes| user.nil? ? '' : app_routes.person_path(user.person) }
  ...
  #              user, 1, :s => 60, :d => 'mm') }
  config.user_avatar_proc = lambda { |user, view| user.nil? ?
                                     view.commontator_gravatar_image_tag(
                                       user, 1, :s => 60, :d => 'mm') : (user.person.avatar.nil? ? view.image_tag(view.icon_filename_for_key("person_avatar"), :size => "32x32") : view.image_tag(view.avatar_url(user.person, user.person.avatar_id, 32)) )}
  ...
  config.user_email_proc = lambda { |user, mailer| user.try(:person).try(:email) || '' }
  ...
  config.thread_read_proc = lambda { |thread, user| user && user.can_view?(thread.commontable) }
  ...
  config.thread_moderator_proc = lambda { |thread, user| false  } #user && user.can_edit?(thread.commontable)
